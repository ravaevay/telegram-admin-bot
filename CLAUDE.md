...
Telegram admin bot for managing ONLYOFFICE test infrastructure: mailbox creation/password reset on an iRedMail server via SSH, and DigitalOcean droplet lifecycle management (create, extend, auto-delete) with DNS A-record automation, live pricing display, cost tracking, creator tagging, automatic snapshots before expiry deletion, and SSH key preference learning. Built with python-telegram-bot 20.3 (async, job-queue) and Python 3.9.
...
- `modules/database.py` — SQLite CRUD for `instances` table (droplet_id, name, ip_address, droplet_type, expiration_date, ssh_key_id, creator_id, creator_username, domain_name, dns_record_id, dns_zone, created_at, price_hourly) and `ssh_key_usage` table (user_id, ssh_key_id, usage_count, last_used). Schema migrations via `_migrate_add_column()`. `get_expiring_instances()` returns `list[dict]`. `record_ssh_key_usage()` / `get_preferred_ssh_keys()` track per-user SSH key preferences.
