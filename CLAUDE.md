...
**Handler flow:**
- `/start` → `start()` shows inline keyboard with 7 actions (mail create, password reset, create droplet, manage droplets, create K8s cluster, manage K8s clusters, create test stand)
- Seven `ConversationHandler`s (mail creation, password reset, droplet creation, droplet management, K8s creation, K8s management, test stand creation) route callback/text input through state machines
...
**Test stand creation conversation states (300–307):** `STAND_SELECT_SERVICE → STAND_SELECT_DS_TAG → STAND_SELECT_SSH_KEY → STAND_SELECT_DNS_ZONE → STAND_INPUT_SUBDOMAIN → STAND_SELECT_TYPE → STAND_SELECT_DURATION → STAND_INPUT_NAME`. Deploys a pre-configured test stand from `services4integration` repo. Creates a DigitalOcean droplet with cloud-init `user_data` that clones the repo and runs the service's `install.sh`. DNS steps are optional (same pattern as droplet creation). Available services configured via `STAND_SERVICES` env var. Separate authorization group `"stand"` (`AUTHORIZED_STAND_USERS`).
...
**Modules:**
- `config.py` — loads `.env` via python-dotenv, builds `SSH_CONFIG` dict and `AUTHORIZED_GROUPS` dict (keyed by `"mail"`, `"droplet"`, `"k8s"`, `"stand"`). Test stand config: `STAND_SERVICES` (list), `STAND_DEFAULT_DS_TAG`, `STAND_DEFAULT_IMAGE`, `STAND_DROPLET_TYPES` (dict)
...
- `modules/database.py` — SQLite CRUD for three tables: `instances` (droplets and test stands), `ssh_key_usage` (per-user SSH key preferences), and `k8s_clusters` (cluster_id TEXT PK, cluster_name, region, version, node_size, node_count, status, endpoint, creator_id, creator_username, expiration_date, created_at, price_hourly, ha). Schema migrations via `_migrate_add_column()`. Both `instances` and `k8s_clusters` have a `platform` column (`TEXT DEFAULT 'telegram'`) to distinguish records created by each bot. `instances` has a `stand_type` column (`TEXT`, NULL for plain droplets, service name for test stands). WAL mode is enabled for safe concurrent access from both bots. `save_instance()` accepts `platform` and `stand_type` parameters. `get_expiring_instances()`, `get_expiring_k8s_clusters()`, and `get_provisioning_k8s_clusters()` accept an optional `platform` filter. K8s functions: `save_k8s_cluster()`, `get_k8s_cluster_by_id/name()`, `get_k8s_clusters_by_creator()`, `update_k8s_cluster_status()`, `delete_k8s_cluster()`, `get_expiring_k8s_clusters()`, `get_provisioning_k8s_clusters()`, `extend_k8s_cluster_expiration()`.
...
- `modules/create_test_instance.py` — DigitalOcean REST API calls (create/delete droplets, list SSH keys/images, DNS record management, size/pricing with 1h TTL cache, snapshot creation with action polling, creator tagging via `_sanitize_tag()`). Droplets created in `fra1` region. `build_stand_user_data(service, ds_tag, service_tag, domain_name)` generates cloud-config for test stand deployment. `create_droplet()` accepts optional `user_data` and `stand_type` parameters.
