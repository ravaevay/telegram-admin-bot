...
**Modules:**
- `config.py` — loads `.env` via python-dotenv, builds `SSH_CONFIG` dict and `AUTHORIZED_GROUPS` dict (keyed by `"mail"`, `"droplet"`, `"k8s"`, `"stand"`). Test stand config: `STAND_SERVICES` (list), `STAND_DEFAULT_DS_TAG`, `STAND_DROPLET_TYPES` (dict)
