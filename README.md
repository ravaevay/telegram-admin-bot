...
Этот бот предназначен для управления почтовыми ящиками, виртуальными машинами (Droplets) и кластерами Kubernetes в DigitalOcean. Он позволяет:
...
│       ├── database.py        # Работа с базой данных (SQLite): дроплеты, K8s, SSH-ключи
│       ├── authorization.py   # Проверка прав доступа
│       ├── mail.py            # Управление почтовыми ящиками
│       ├── notifications.py   # Уведомления в канал (дроплеты и K8s)
...
-	База данных: SQLite (instances.db — таблицы `instances`, `k8s_clusters`, `ssh_key_usage`)
