...
- ✅ Разворачивать тестовые стенды через Gitea Actions из [stands-for-connectors](https://git.onlyoffice.com/ONLYOFFICE-DevOps/stands-for-connectors) (WordPress, Moodle, Odoo, Drupal, Jira, Plone и др.)
- ✅ Задавать параметры workflow стенда (версия приложения, ветка/версия коннектора) с дефолтами
- ✅ Отслеживать статус деплоя стенда и уведомлять о готовности (HTTPS через Let's Encrypt)
- ✅ Продлевать и удалять стенды (удаление — через destroy-workflow, terraform destroy)
...
│       ├── gitea_stands.py        # Тестовые стенды через Gitea Actions (каталог, dispatch, статус ранов)
...
│   └── test_stand.py                 # Тесты тестовых стендов (каталог, Gitea API, БД)
│
...
# Тестовые стенды — Gitea Actions (опционально; без GITEA_TOKEN функции стендов отключены)
# GITEA_TOKEN=your-gitea-api-token   # токен с правом write на репозиторий стендов
# GITEA_URL=https://git.onlyoffice.com
# STANDS_REPO_OWNER=ONLYOFFICE-DevOps
# STANDS_REPO_NAME=stands-for-connectors
# STAND_DOMAIN=onlyoffice.fun
```
...
6.	Создание тестового стенда (Gitea Actions)
	-	Бот предлагает выбрать сервис из [stands-for-connectors](https://git.onlyoffice.com/ONLYOFFICE-DevOps/stands-for-connectors) (WordPress, Moodle, Odoo, Drupal, FineBI, Plone, Jira, Mattermost, Dropbox, HumHub, ownCloud, Alfresco)
	-	Запрашивает субдомен — стенд будет доступен как `<субдомен>.onlyoffice.fun` (HTTPS через Let's Encrypt)
	-	Последовательно запрашивает параметры workflow (версия приложения, ветка/версия коннектора) — у каждого есть кнопка «По умолчанию»
	-	Показывает выбор длительности аренды
	-	Диспатчит deploy-workflow в репозитории стендов (terraform + ansible на стороне Gitea Actions)
	-	Деплой занимает 10–30 минут; фоновая задача (каждые 60с) следит за раном и уведомляет о готовности или ошибке (со ссылкой на лог рана)
	-	⚠️ У всех сервисов, кроме FineBI, общий terraform state на сервис: второй стенд того же сервиса пересоздаст первый
7.	Управление тестовыми стендами («Мои стенды»)
	-	Отображение всех стендов пользователя с адресом, статусом и сроком действия
	-	Бот отправляет уведомление за 24 часа до удаления (кнопки продления/удаления)
	-	Удаление — диспатч того же workflow с `mode=destroy`; запись удаляется из БД только после успешного destroy-рана
	-	Просроченные стенды удаляются автоматически (снэпшот не создаётся); при неудачном destroy стенд помечается «ошибка удаления» и удаление ретраится

## 📌 **Дополнительная информация**
-	Поддерживаемые образы DigitalOcean: Ubuntu, CentOS, Fedora
-	Подключение к почте: IMAP (143, STARTTLS), SMTP (587, STARTTLS)
-	База данных: SQLite (instances.db — таблицы `instances`, `k8s_clusters`, `stands`, `ssh_key_usage`; WAL-режим для параллельного доступа; колонка `platform` для разделения ресурсов между Telegram и Mattermost)
-	DNS: автоматическое создание/удаление A-записей через DigitalOcean DNS API (для дроплетов); DNS стендов управляется terraform-модулем в репозитории стендов
...
-	Уведомления: в канал отправляются с @username вместо числового ID; поддерживаются события дроплетов, K8s кластеров и стендов (created/ready/errored/extended/deleted/auto_deleted/destroy_failed)
-	K8s кластеры: регион `fra1`, поддерживается HA-режим (через API)
-	Тестовые стенды: деплоятся Gitea Actions workflow'ами (`deploy-<service>.yml`, terraform + ansible) в репозитории `stands-for-connectors`; бот диспатчит workflow через Gitea API и отслеживает раны; отдельная группа авторизации `stand`; требуется `GITEA_TOKEN` с правом write на репозиторий
